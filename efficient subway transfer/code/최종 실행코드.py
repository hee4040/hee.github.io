...
def process():
    # GUI 설정
    window = tk.Tk()
    window.title("지하철 소요 시간 계산기")

    ttk.Label(window, text="출발역").grid(row=0, column=0, padx=10, pady=10)
    start_entry = ttk.Entry(window, width=33)
    start_entry.grid(row=0, column=1, padx=10, pady=10)

    ttk.Label(window, text="환승역").grid(row=1, column=0, padx=10, pady=10)
    transfer_entry = ttk.Entry(window, width=33)
    transfer_entry.grid(row=1, column=1, padx=10, pady=10)

    ttk.Label(window, text="도착역").grid(row=2, column=0, padx=10, pady=10)
    end_entry = ttk.Entry(window, width=33)
    ...
    def show_results():
        ...
        transfer_station = transfer_entry.get()
        ...
        try:
            ...
            if train_info is not None:
                ...
                if closest_train:
                    ...
                    messagebox.showerror("Error", f"{start_station}에서 출발할 열차 정보를 찾을 수 없습니다.")
                    return
            else:
                messagebox.showerror("Error", "실시간 열차 정보를 가져오지 못했습니다.")
                return
            ...
            if transfer_train_info is not None:
                ...
                if closest_transfer_train:
                    ...
                    messagebox.showinfo(
                        ...
                        f"환승역 {transfer_station}의 실제 열차 도착 시간: {closest_transfer_train_time.strftime('%H:%M')}\n"
                        f"도착역 {end_station}의 예상 도착 시간: {end_time.strftime('%H:%M')}"
                    )
                    ...
                    messagebox.showerror("Error", f"{transfer_station}에서 출발할 열차 정보를 찾을 수 없습니다.")
            else:
                messagebox.showerror("Error", "실시간 열차 정보를 가져오지 못했습니다.")
    ...
    # 결과 표시 버튼
    result_button2 = ttk.Button(window, text="실제 도착 시간 표시", command=show_results)
    result_button2.grid(row=4, column=0, columnspan=2, padx=10, pady=10)

    window.mainloop()

window = Tk()
window.title('road trip')
road_image = PhotoImage(file='c:\\road.png')
road = Button(window, image=road_image, command=process)
road.grid(row=0, column=0, padx=20, pady=5)
window.mainloop()